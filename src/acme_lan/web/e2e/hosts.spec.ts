...
// Selects are Catalyst listboxes, not native <select>s: click the button, then the
// option (which renders in a portal at the document root, hence page-level lookup).
async function choose(page: Page, listbox: Locator, optionName: string | RegExp) {
  await listbox.click()
...
test.describe('managed hosts', () => {
  ...
  test('warns when choosing the local CSR source in the modal', async ({ page }) => {
    ...
    await choose(page, modal.getByTestId('csr-source-select'), 'local (acme-lan holds key)')
    ...
    await choose(page, modal.getByTestId('csr-source-select'), 'device (key stays on device)')
  ...
  test('renders plugin-specific config fields that change with plugin/mode', async ({ page }) => {
    ...
    await choose(page, modal.getByTestId('plugin-select'), 'local')
    await choose(page, modal.getByTestId('csr-source-select'), 'device (key stays on device)')
    ...
    await choose(page, modal.getByTestId('plugin-select'), 'ssh')
